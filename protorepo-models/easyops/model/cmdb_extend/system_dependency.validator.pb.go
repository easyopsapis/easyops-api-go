...
var _regex_SystemDependency_ObjectId = regexp.MustCompile(`^[a-zA-Z_][0-9a-zA-Z_]{0,31}$`)
var _regex_SystemDependency_InstanceId = regexp.MustCompile(`^[0-9a-z]{13}$`)

func (this *SystemDependency) Validate() error {
	if !_regex_SystemDependency_ObjectId.MatchString(this.ObjectId) {
	...
	if !_regex_SystemDependency_InstanceId.MatchString(this.InstanceId) {
	...
	for _, item := range this.Subsystems {
		if item != nil {
			if err := github_com_mwitkow_go_proto_validators.CallValidatorIfExists(item); err != nil {
				return github_com_mwitkow_go_proto_validators.FieldError("Subsystems", err)
			}
		}
	}
	return nil
