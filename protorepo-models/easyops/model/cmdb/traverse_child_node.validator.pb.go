...
var _regex_TraverseChildNode_ParentOut = regexp.MustCompile(`^[a-zA-Z_][0-9a-zA-Z_]{0,31}$`)

func (this *TraverseChildNode) Validate() error {
	if !_regex_TraverseChildNode_ParentOut.MatchString(this.ParentOut) {
		return github_com_mwitkow_go_proto_validators.FieldError("ParentOut", fmt.Errorf(`value '%v' must be a string conforming to regex "^[a-zA-Z_][0-9a-zA-Z_]{0,31}$"`, this.ParentOut))
	}
	if this.Query != nil {
